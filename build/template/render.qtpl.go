// Code generated by qtc from "render.qtpl". DO NOT EDIT.
// See https://github.com/valyala/quicktemplate for details.

//line build/template/render.qtpl:2
package template

//line build/template/render.qtpl:2
import (
	"fmt"

	"github.com/andrewpillar/thrall/build"
	"github.com/andrewpillar/thrall/template"
)

//line build/template/render.qtpl:11
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line build/template/render.qtpl:11
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line build/template/render.qtpl:11
func StreamRenderStatusNav(qw422016 *qt422016.Writer, uri, status string) {
//line build/template/render.qtpl:11
	qw422016.N().S(` `)
//line build/template/render.qtpl:12
	qw422016.N().S(`<ul class="panel-nav"><li><a href="`)
//line build/template/render.qtpl:15
	qw422016.E().S(uri)
//line build/template/render.qtpl:15
	qw422016.N().S(`"`)
//line build/template/render.qtpl:15
	if status == "" {
//line build/template/render.qtpl:15
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:15
	}
//line build/template/render.qtpl:15
	qw422016.N().S(`>`)
//line build/template/render.qtpl:16
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M22.688 18.984c0.422 0.281 0.422 0.984-0.094 1.406l-2.297 2.297c-0.422 0.422-0.984 0.422-1.406 0l-9.094-9.094c-2.297 0.891-4.969 0.422-6.891-1.5-2.016-2.016-2.531-5.016-1.313-7.406l4.406 4.313 3-3-4.313-4.313c2.391-1.078 5.391-0.703 7.406 1.313 1.922 1.922 2.391 4.594 1.5 6.891z"></path>
</svg>
`)
//line build/template/render.qtpl:16
	qw422016.N().S(`<span>All</span></a></li><li><a href="?status=queued"`)
//line build/template/render.qtpl:20
	if status == "queued" {
//line build/template/render.qtpl:20
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:20
	}
//line build/template/render.qtpl:20
	qw422016.N().S(`>`)
//line build/template/render.qtpl:21
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M12 11.484l3.984-3.984v-3.516h-7.969v3.516zM15.984 16.5l-3.984-3.984-3.984 3.984v3.516h7.969v-3.516zM6 2.016h12v6l-3.984 3.984 3.984 3.984v6h-12v-6l3.984-3.984-3.984-3.984v-6z"></path>
</svg>
`)
//line build/template/render.qtpl:21
	qw422016.N().S(`<span>Queued</span></a></li><li><a href="?status=running"`)
//line build/template/render.qtpl:25
	if status == "running" {
//line build/template/render.qtpl:25
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:25
	}
//line build/template/render.qtpl:25
	qw422016.N().S(`>`)
//line build/template/render.qtpl:26
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M21.984 12c0 5.156-3.938 9.422-8.953 9.938v-2.016c3.938-0.516 6.984-3.891 6.984-7.922s-3.047-7.406-6.984-7.922v-2.016c5.016 0.516 8.953 4.781 8.953 9.938zM5.672 19.734l1.406-1.406c1.125 0.844 2.484 1.406 3.938 1.594v2.016c-2.016-0.188-3.844-0.984-5.344-2.203zM4.078 12.984c0.188 1.453 0.75 2.813 1.594 3.891l-1.406 1.453c-1.219-1.5-2.016-3.328-2.203-5.344h2.016zM5.672 7.078c-0.844 1.125-1.406 2.484-1.594 3.938h-2.016c0.188-2.016 0.984-3.844 2.203-5.344zM11.016 4.078c-1.453 0.188-2.813 0.75-3.938 1.594l-1.406-1.406c1.5-1.219 3.328-2.016 5.344-2.203v2.016zM13.031 9.797l2.953 2.203c-2.007 1.493-4.007 2.993-6 4.5z"></path>
</svg>
`)
//line build/template/render.qtpl:26
	qw422016.N().S(`<span>Running</span></a></li><li><a href="?status=passed"`)
//line build/template/render.qtpl:30
	if status == "passed" {
//line build/template/render.qtpl:30
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:30
	}
//line build/template/render.qtpl:30
	qw422016.N().S(`>`)
//line build/template/render.qtpl:31
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M9 16.172l10.594-10.594 1.406 1.406-12 12-5.578-5.578 1.406-1.406z"></path>
</svg>
`)
//line build/template/render.qtpl:31
	qw422016.N().S(`<span>Passed</span></a></li><li><a href="?status=failed"`)
//line build/template/render.qtpl:35
	if status == "failed" {
//line build/template/render.qtpl:35
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:35
	}
//line build/template/render.qtpl:35
	qw422016.N().S(`>`)
//line build/template/render.qtpl:36
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M18.984 6.422l-5.578 5.578 5.578 5.578-1.406 1.406-5.578-5.578-5.578 5.578-1.406-1.406 5.578-5.578-5.578-5.578 1.406-1.406 5.578 5.578 5.578-5.578z"></path>
</svg>
`)
//line build/template/render.qtpl:36
	qw422016.N().S(`<span>Failed</span></a></li><li><a href="?status=killed"`)
//line build/template/render.qtpl:40
	if status == "killed" {
//line build/template/render.qtpl:40
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:40
	}
//line build/template/render.qtpl:40
	qw422016.N().S(`>`)
//line build/template/render.qtpl:41
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M12.984 12.984v-6h-1.969v6h1.969zM12 17.297c0.703 0 1.313-0.609 1.313-1.313s-0.609-1.266-1.313-1.266-1.313 0.563-1.313 1.266 0.609 1.313 1.313 1.313zM15.75 3l5.25 5.25v7.5l-5.25 5.25h-7.5l-5.25-5.25v-7.5l5.25-5.25h7.5z"></path>
</svg>
`)
//line build/template/render.qtpl:41
	qw422016.N().S(`<span>Killed</span></a></li><li><a href="?status=timed_out"`)
//line build/template/render.qtpl:45
	if status == "timed_out" {
//line build/template/render.qtpl:45
		qw422016.N().S(`class="active"`)
//line build/template/render.qtpl:45
	}
//line build/template/render.qtpl:45
	qw422016.N().S(`>`)
//line build/template/render.qtpl:46
	qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M12 20.016c3.891 0 6.984-3.141 6.984-7.031s-3.094-6.984-6.984-6.984-6.984 3.094-6.984 6.984 3.094 7.031 6.984 7.031zM19.031 7.406c1.219 1.547 1.969 3.469 1.969 5.578 0 4.969-4.031 9-9 9s-9-4.031-9-9 4.031-9 9-9c2.109 0 4.078 0.797 5.625 2.016l1.406-1.453c0.516 0.422 0.984 0.891 1.406 1.406zM11.016 14.016v-6h1.969v6h-1.969zM15 0.984v2.016h-6v-2.016h6z"></path>
</svg>
`)
//line build/template/render.qtpl:46
	qw422016.N().S(`<span>Timed Out</span></a></li></ul>`)
//line build/template/render.qtpl:50
	qw422016.N().S(` `)
//line build/template/render.qtpl:51
}

//line build/template/render.qtpl:51
func WriteRenderStatusNav(qq422016 qtio422016.Writer, uri, status string) {
//line build/template/render.qtpl:51
	qw422016 := qt422016.AcquireWriter(qq422016)
//line build/template/render.qtpl:51
	StreamRenderStatusNav(qw422016, uri, status)
//line build/template/render.qtpl:51
	qt422016.ReleaseWriter(qw422016)
//line build/template/render.qtpl:51
}

//line build/template/render.qtpl:51
func RenderStatusNav(uri, status string) string {
//line build/template/render.qtpl:51
	qb422016 := qt422016.AcquireByteBuffer()
//line build/template/render.qtpl:51
	WriteRenderStatusNav(qb422016, uri, status)
//line build/template/render.qtpl:51
	qs422016 := string(qb422016.B)
//line build/template/render.qtpl:51
	qt422016.ReleaseByteBuffer(qb422016)
//line build/template/render.qtpl:51
	return qs422016
//line build/template/render.qtpl:51
}

//line build/template/render.qtpl:53
func StreamRenderArtifactTable(qw422016 *qt422016.Writer, aa []*build.Artifact, uri, search string) {
//line build/template/render.qtpl:53
	qw422016.N().S(` `)
//line build/template/render.qtpl:54
	if len(aa) == 0 && search == "" {
//line build/template/render.qtpl:54
		qw422016.N().S(` <div class="panel-message muted">No artifacts have been collected from this build.</div> `)
//line build/template/render.qtpl:56
	} else {
//line build/template/render.qtpl:56
		qw422016.N().S(` <div class="panel-header">`)
//line build/template/render.qtpl:57
		template.StreamRenderSearch(qw422016, uri, search, "Find an artifact...")
//line build/template/render.qtpl:57
		qw422016.N().S(`</div> `)
//line build/template/render.qtpl:58
		if len(aa) == 0 && search != "" {
//line build/template/render.qtpl:58
			qw422016.N().S(` <div class="panel-message muted">No results found.</div> `)
//line build/template/render.qtpl:60
		} else {
//line build/template/render.qtpl:60
			qw422016.N().S(` <table class="table"> <thead> <tr> <th>SOURCE</th> <th>NAME</th> <tH>SIZE</th> <th>MD5</th> <th>SHA256</th> </tr> </thead> <tbody> `)
//line build/template/render.qtpl:72
			for _, a := range aa {
//line build/template/render.qtpl:72
				qw422016.N().S(` <tr> <td><span class="code">`)
//line build/template/render.qtpl:74
				qw422016.E().S(a.Source)
//line build/template/render.qtpl:74
				qw422016.N().S(`</span></td> <td><a href="`)
//line build/template/render.qtpl:75
				qw422016.E().S(a.Endpoint("download", a.Name))
//line build/template/render.qtpl:75
				qw422016.N().S(`">`)
//line build/template/render.qtpl:75
				qw422016.E().S(a.Name)
//line build/template/render.qtpl:75
				qw422016.N().S(`</a></td> <td>`)
//line build/template/render.qtpl:76
				qw422016.E().S(template.RenderSize(a.Size.Int64))
//line build/template/render.qtpl:76
				qw422016.N().S(`</td> <td> <span class="code"> `)
//line build/template/render.qtpl:79
				if len(a.MD5) == 0 {
//line build/template/render.qtpl:79
					qw422016.N().S(` -- `)
//line build/template/render.qtpl:81
				} else {
//line build/template/render.qtpl:81
					qw422016.N().S(` `)
//line build/template/render.qtpl:82
					qw422016.E().S(fmt.Sprintf("%x", a.MD5))
//line build/template/render.qtpl:82
					qw422016.N().S(` `)
//line build/template/render.qtpl:83
				}
//line build/template/render.qtpl:83
				qw422016.N().S(` </span> </td> <td> <span class="code"> `)
//line build/template/render.qtpl:88
				if len(a.SHA256) == 0 {
//line build/template/render.qtpl:88
					qw422016.N().S(` -- `)
//line build/template/render.qtpl:90
				} else {
//line build/template/render.qtpl:90
					qw422016.N().S(` `)
//line build/template/render.qtpl:91
					qw422016.E().S(fmt.Sprintf("%x", a.SHA256))
//line build/template/render.qtpl:91
					qw422016.N().S(` `)
//line build/template/render.qtpl:92
				}
//line build/template/render.qtpl:92
				qw422016.N().S(` </span> </td> </tr> `)
//line build/template/render.qtpl:96
			}
//line build/template/render.qtpl:96
			qw422016.N().S(` </tbody> </table> `)
//line build/template/render.qtpl:99
		}
//line build/template/render.qtpl:99
		qw422016.N().S(` `)
//line build/template/render.qtpl:100
	}
//line build/template/render.qtpl:100
	qw422016.N().S(` `)
//line build/template/render.qtpl:101
}

//line build/template/render.qtpl:101
func WriteRenderArtifactTable(qq422016 qtio422016.Writer, aa []*build.Artifact, uri, search string) {
//line build/template/render.qtpl:101
	qw422016 := qt422016.AcquireWriter(qq422016)
//line build/template/render.qtpl:101
	StreamRenderArtifactTable(qw422016, aa, uri, search)
//line build/template/render.qtpl:101
	qt422016.ReleaseWriter(qw422016)
//line build/template/render.qtpl:101
}

//line build/template/render.qtpl:101
func RenderArtifactTable(aa []*build.Artifact, uri, search string) string {
//line build/template/render.qtpl:101
	qb422016 := qt422016.AcquireByteBuffer()
//line build/template/render.qtpl:101
	WriteRenderArtifactTable(qb422016, aa, uri, search)
//line build/template/render.qtpl:101
	qs422016 := string(qb422016.B)
//line build/template/render.qtpl:101
	qt422016.ReleaseByteBuffer(qb422016)
//line build/template/render.qtpl:101
	return qs422016
//line build/template/render.qtpl:101
}

//line build/template/render.qtpl:103
func StreamRenderObjectTable(qw422016 *qt422016.Writer, oo []*build.Object) {
//line build/template/render.qtpl:103
	qw422016.N().S(` <table class="table"> <thead> <tr> <th>SOURCE</th> <th>NAME</th> <th>TYPE</th> <th>MD5</th> <th>SHA256</th> <th></th> </tr> </thead> <tbody> `)
//line build/template/render.qtpl:116
	for _, o := range oo {
//line build/template/render.qtpl:116
		qw422016.N().S(` <tr> <td> `)
//line build/template/render.qtpl:119
		if o != nil && !o.Object.DeletedAt.Valid {
//line build/template/render.qtpl:119
			qw422016.N().S(` <a href="`)
//line build/template/render.qtpl:120
			qw422016.E().S(o.Object.Endpoint())
//line build/template/render.qtpl:120
			qw422016.N().S(`">`)
//line build/template/render.qtpl:120
			qw422016.E().S(o.Source)
//line build/template/render.qtpl:120
			qw422016.N().S(`</a> `)
//line build/template/render.qtpl:121
		} else {
//line build/template/render.qtpl:121
			qw422016.N().S(` <a title="Object not found"><strike>`)
//line build/template/render.qtpl:122
			qw422016.E().S(o.Source)
//line build/template/render.qtpl:122
			qw422016.N().S(`</strike></a> `)
//line build/template/render.qtpl:123
		}
//line build/template/render.qtpl:123
		qw422016.N().S(` </td> <td><span class="code">`)
//line build/template/render.qtpl:125
		qw422016.E().S(o.Name)
//line build/template/render.qtpl:125
		qw422016.N().S(`</span></td> <td> `)
//line build/template/render.qtpl:127
		if o.Object != nil {
//line build/template/render.qtpl:127
			qw422016.N().S(` <span class="code">`)
//line build/template/render.qtpl:128
			qw422016.E().S(o.Object.Type)
//line build/template/render.qtpl:128
			qw422016.N().S(`</span> `)
//line build/template/render.qtpl:129
		} else {
//line build/template/render.qtpl:129
			qw422016.N().S(` <span class="code">--</span> `)
//line build/template/render.qtpl:131
		}
//line build/template/render.qtpl:131
		qw422016.N().S(` </td> <td> `)
//line build/template/render.qtpl:134
		if o != nil {
//line build/template/render.qtpl:134
			qw422016.N().S(` <span class="code">`)
//line build/template/render.qtpl:135
			qw422016.E().S(fmt.Sprintf("%x", o.Object.MD5))
//line build/template/render.qtpl:135
			qw422016.N().S(`</span> `)
//line build/template/render.qtpl:136
		} else {
//line build/template/render.qtpl:136
			qw422016.N().S(` <span class="code">--</span> `)
//line build/template/render.qtpl:138
		}
//line build/template/render.qtpl:138
		qw422016.N().S(` </td> <td> `)
//line build/template/render.qtpl:141
		if o != nil {
//line build/template/render.qtpl:141
			qw422016.N().S(` <span class="code">`)
//line build/template/render.qtpl:142
			qw422016.E().S(fmt.Sprintf("%x", o.Object.SHA256))
//line build/template/render.qtpl:142
			qw422016.N().S(`</span> `)
//line build/template/render.qtpl:143
		} else {
//line build/template/render.qtpl:143
			qw422016.N().S(` <span class="code">--</span> `)
//line build/template/render.qtpl:145
		}
//line build/template/render.qtpl:145
		qw422016.N().S(` </td> <td class="align-right"> `)
//line build/template/render.qtpl:148
		if o.Placed {
//line build/template/render.qtpl:148
			qw422016.N().S(` <span class="pill pill-green">`)
//line build/template/render.qtpl:149
			qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M9 16.172l10.594-10.594 1.406 1.406-12 12-5.578-5.578 1.406-1.406z"></path>
</svg>
`)
//line build/template/render.qtpl:149
			qw422016.N().S(` Placed</span> `)
//line build/template/render.qtpl:150
		} else {
//line build/template/render.qtpl:150
			qw422016.N().S(` <span class="pill pill-red">`)
//line build/template/render.qtpl:151
			qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M18.984 6.422l-5.578 5.578 5.578 5.578-1.406 1.406-5.578-5.578-5.578 5.578-1.406-1.406 5.578-5.578-5.578-5.578 1.406-1.406 5.578 5.578 5.578-5.578z"></path>
</svg>
`)
//line build/template/render.qtpl:151
			qw422016.N().S(` Not Placed</span> `)
//line build/template/render.qtpl:152
		}
//line build/template/render.qtpl:152
		qw422016.N().S(` </td> </tr> `)
//line build/template/render.qtpl:155
	}
//line build/template/render.qtpl:155
	qw422016.N().S(` </tbody> </table> `)
//line build/template/render.qtpl:158
}

//line build/template/render.qtpl:158
func WriteRenderObjectTable(qq422016 qtio422016.Writer, oo []*build.Object) {
//line build/template/render.qtpl:158
	qw422016 := qt422016.AcquireWriter(qq422016)
//line build/template/render.qtpl:158
	StreamRenderObjectTable(qw422016, oo)
//line build/template/render.qtpl:158
	qt422016.ReleaseWriter(qw422016)
//line build/template/render.qtpl:158
}

//line build/template/render.qtpl:158
func RenderObjectTable(oo []*build.Object) string {
//line build/template/render.qtpl:158
	qb422016 := qt422016.AcquireByteBuffer()
//line build/template/render.qtpl:158
	WriteRenderObjectTable(qb422016, oo)
//line build/template/render.qtpl:158
	qs422016 := string(qb422016.B)
//line build/template/render.qtpl:158
	qt422016.ReleaseByteBuffer(qb422016)
//line build/template/render.qtpl:158
	return qs422016
//line build/template/render.qtpl:158
}

//line build/template/render.qtpl:160
func StreamRenderTagTable(qw422016 *qt422016.Writer, tt []*build.Tag, csrf string) {
//line build/template/render.qtpl:160
	qw422016.N().S(` <table class="table"> <thead> <tr> <th>TAG</th> <th>TAGGED BY</th> <th></th> </tr> </thead> <tbody> `)
//line build/template/render.qtpl:170
	for _, t := range tt {
//line build/template/render.qtpl:170
		qw422016.N().S(` <tr> <td><a href="/?tag=`)
//line build/template/render.qtpl:172
		qw422016.E().S(t.Name)
//line build/template/render.qtpl:172
		qw422016.N().S(`" class="pill pill-light">`)
//line build/template/render.qtpl:172
		qw422016.E().S(t.Name)
//line build/template/render.qtpl:172
		qw422016.N().S(`</a></td> <td>`)
//line build/template/render.qtpl:173
		qw422016.E().S(t.User.Username)
//line build/template/render.qtpl:173
		qw422016.N().S(` &lt;`)
//line build/template/render.qtpl:173
		qw422016.E().S(t.User.Email)
//line build/template/render.qtpl:173
		qw422016.N().S(`&gt;</td> <td class="align-right"> <form method="POST" action="`)
//line build/template/render.qtpl:175
		qw422016.E().S(t.Endpoint())
//line build/template/render.qtpl:175
		qw422016.N().S(`"> `)
//line build/template/render.qtpl:176
		qw422016.N().S(csrf)
//line build/template/render.qtpl:176
		qw422016.N().S(` <input type="hidden" name="_method" value="DELETE"> <button type="submit" class="btn btn-danger">Delete</button> </form> </td> </tr> `)
//line build/template/render.qtpl:182
	}
//line build/template/render.qtpl:182
	qw422016.N().S(` </tbody> </table> `)
//line build/template/render.qtpl:185
}

//line build/template/render.qtpl:185
func WriteRenderTagTable(qq422016 qtio422016.Writer, tt []*build.Tag, csrf string) {
//line build/template/render.qtpl:185
	qw422016 := qt422016.AcquireWriter(qq422016)
//line build/template/render.qtpl:185
	StreamRenderTagTable(qw422016, tt, csrf)
//line build/template/render.qtpl:185
	qt422016.ReleaseWriter(qw422016)
//line build/template/render.qtpl:185
}

//line build/template/render.qtpl:185
func RenderTagTable(tt []*build.Tag, csrf string) string {
//line build/template/render.qtpl:185
	qb422016 := qt422016.AcquireByteBuffer()
//line build/template/render.qtpl:185
	WriteRenderTagTable(qb422016, tt, csrf)
//line build/template/render.qtpl:185
	qs422016 := string(qb422016.B)
//line build/template/render.qtpl:185
	qt422016.ReleaseByteBuffer(qb422016)
//line build/template/render.qtpl:185
	return qs422016
//line build/template/render.qtpl:185
}

//line build/template/render.qtpl:187
func StreamRenderVariableTable(qw422016 *qt422016.Writer, vv []*build.Variable) {
//line build/template/render.qtpl:187
	qw422016.N().S(` <table class="table"> <thead> <tr> <th>KEY</th> <th>VALUE</th> <th>FROM MANIFEST</th> </tr> </thead> <tbody> `)
//line build/template/render.qtpl:197
	for _, v := range vv {
//line build/template/render.qtpl:197
		qw422016.N().S(` <tr> <td><span class="code">`)
//line build/template/render.qtpl:199
		qw422016.E().S(v.Key)
//line build/template/render.qtpl:199
		qw422016.N().S(`</span></td> <td><span class="code">`)
//line build/template/render.qtpl:200
		qw422016.E().S(v.Value)
//line build/template/render.qtpl:200
		qw422016.N().S(`</span></td> <td>`)
//line build/template/render.qtpl:201
		if v.VariableID.Valid {
//line build/template/render.qtpl:201
			qw422016.N().S(`False`)
//line build/template/render.qtpl:201
		} else {
//line build/template/render.qtpl:201
			qw422016.N().S(`True`)
//line build/template/render.qtpl:201
		}
//line build/template/render.qtpl:201
		qw422016.N().S(`</td> </tr> `)
//line build/template/render.qtpl:203
	}
//line build/template/render.qtpl:203
	qw422016.N().S(` </tbody> </table> `)
//line build/template/render.qtpl:206
}

//line build/template/render.qtpl:206
func WriteRenderVariableTable(qq422016 qtio422016.Writer, vv []*build.Variable) {
//line build/template/render.qtpl:206
	qw422016 := qt422016.AcquireWriter(qq422016)
//line build/template/render.qtpl:206
	StreamRenderVariableTable(qw422016, vv)
//line build/template/render.qtpl:206
	qt422016.ReleaseWriter(qw422016)
//line build/template/render.qtpl:206
}

//line build/template/render.qtpl:206
func RenderVariableTable(vv []*build.Variable) string {
//line build/template/render.qtpl:206
	qb422016 := qt422016.AcquireByteBuffer()
//line build/template/render.qtpl:206
	WriteRenderVariableTable(qb422016, vv)
//line build/template/render.qtpl:206
	qs422016 := string(qb422016.B)
//line build/template/render.qtpl:206
	qt422016.ReleaseByteBuffer(qb422016)
//line build/template/render.qtpl:206
	return qs422016
//line build/template/render.qtpl:206
}

//line build/template/render.qtpl:208
func StreamRenderKeyTable(qw422016 *qt422016.Writer, kk []*build.Key) {
//line build/template/render.qtpl:208
	qw422016.N().S(` <table class="table"> <thead> <tr> <th>KEY</th> <th>LOCATION</th> </tr> </thead> <tbody> `)
//line build/template/render.qtpl:217
	for _, k := range kk {
//line build/template/render.qtpl:217
		qw422016.N().S(` <tr> <td><span class="code">`)
//line build/template/render.qtpl:219
		qw422016.E().S(k.Name)
//line build/template/render.qtpl:219
		qw422016.N().S(`</span></td> <td><span class="code">`)
//line build/template/render.qtpl:220
		qw422016.E().S(k.Location)
//line build/template/render.qtpl:220
		qw422016.N().S(`</span></td> </tr> `)
//line build/template/render.qtpl:222
	}
//line build/template/render.qtpl:222
	qw422016.N().S(` </tbody> </table> `)
//line build/template/render.qtpl:225
}

//line build/template/render.qtpl:225
func WriteRenderKeyTable(qq422016 qtio422016.Writer, kk []*build.Key) {
//line build/template/render.qtpl:225
	qw422016 := qt422016.AcquireWriter(qq422016)
//line build/template/render.qtpl:225
	StreamRenderKeyTable(qw422016, kk)
//line build/template/render.qtpl:225
	qt422016.ReleaseWriter(qw422016)
//line build/template/render.qtpl:225
}

//line build/template/render.qtpl:225
func RenderKeyTable(kk []*build.Key) string {
//line build/template/render.qtpl:225
	qb422016 := qt422016.AcquireByteBuffer()
//line build/template/render.qtpl:225
	WriteRenderKeyTable(qb422016, kk)
//line build/template/render.qtpl:225
	qs422016 := string(qb422016.B)
//line build/template/render.qtpl:225
	qt422016.ReleaseByteBuffer(qb422016)
//line build/template/render.qtpl:225
	return qs422016
//line build/template/render.qtpl:225
}
