// This file is automatically generated by qtc from "index.qtpl".
// See https://github.com/valyala/quicktemplate for details.

//line template/image/index.qtpl:2
package image

//line template/image/index.qtpl:2
import (
	"github.com/andrewpillar/thrall/model"
	"github.com/andrewpillar/thrall/template"
)

//line template/image/index.qtpl:8
import (
	qtio422016 "io"

	qt422016 "github.com/valyala/quicktemplate"
)

//line template/image/index.qtpl:8
var (
	_ = qtio422016.Copy
	_ = qt422016.AcquireByteBuffer
)

//line template/image/index.qtpl:9
type IndexPage struct {
	template.BasePage

	CSRF      string
	Paginator model.Paginator
	Images    []*model.Image
	Search    string
}

//line template/image/index.qtpl:20
func (p *IndexPage) StreamTitle(qw422016 *qt422016.Writer) {
	//line template/image/index.qtpl:20
	qw422016.N().S(` Images - Thrall `)
//line template/image/index.qtpl:22
}

//line template/image/index.qtpl:22
func (p *IndexPage) WriteTitle(qq422016 qtio422016.Writer) {
	//line template/image/index.qtpl:22
	qw422016 := qt422016.AcquireWriter(qq422016)
	//line template/image/index.qtpl:22
	p.StreamTitle(qw422016)
	//line template/image/index.qtpl:22
	qt422016.ReleaseWriter(qw422016)
//line template/image/index.qtpl:22
}

//line template/image/index.qtpl:22
func (p *IndexPage) Title() string {
	//line template/image/index.qtpl:22
	qb422016 := qt422016.AcquireByteBuffer()
	//line template/image/index.qtpl:22
	p.WriteTitle(qb422016)
	//line template/image/index.qtpl:22
	qs422016 := string(qb422016.B)
	//line template/image/index.qtpl:22
	qt422016.ReleaseByteBuffer(qb422016)
	//line template/image/index.qtpl:22
	return qs422016
//line template/image/index.qtpl:22
}

//line template/image/index.qtpl:24
func (p *IndexPage) StreamBody(qw422016 *qt422016.Writer) {
	//line template/image/index.qtpl:24
	qw422016.N().S(` <div class="panel"> `)
	//line template/image/index.qtpl:26
	if len(p.Images) == 0 && p.Search == "" {
		//line template/image/index.qtpl:26
		qw422016.N().S(` <div class="panel-message muted">Upload custom images to use as build environments.</div> `)
		//line template/image/index.qtpl:28
	} else {
		//line template/image/index.qtpl:28
		qw422016.N().S(` <div class="panel-header">`)
		//line template/image/index.qtpl:29
		template.StreamRenderSearch(qw422016, p.URL.Path, p.Search, "Find an image...")
		//line template/image/index.qtpl:29
		qw422016.N().S(`</div> `)
		//line template/image/index.qtpl:30
		if len(p.Images) == 0 && p.Search != "" {
			//line template/image/index.qtpl:30
			qw422016.N().S(` <div class="panel-message muted">No results found.</div> `)
			//line template/image/index.qtpl:32
		} else {
			//line template/image/index.qtpl:32
			qw422016.N().S(` <table class="table"> <thead> <tr> <th>NAME</th> <th>NAMESPACE</th> <th></th> <th></th> </tr> </thead> <tbody> `)
			//line template/image/index.qtpl:43
			for _, i := range p.Images {
				//line template/image/index.qtpl:43
				qw422016.N().S(` <tr> <td>`)
				//line template/image/index.qtpl:45
				qw422016.E().S(i.Name)
				//line template/image/index.qtpl:45
				qw422016.N().S(`</td> <td> `)
				//line template/image/index.qtpl:47
				if i.Namespace != nil {
					//line template/image/index.qtpl:47
					qw422016.N().S(` <a href="`)
					//line template/image/index.qtpl:48
					qw422016.E().S(i.Namespace.UIEndpoint())
					//line template/image/index.qtpl:48
					qw422016.N().S(`">`)
					//line template/image/index.qtpl:48
					qw422016.E().S(i.Namespace.Path)
					//line template/image/index.qtpl:48
					qw422016.N().S(`</a> `)
					//line template/image/index.qtpl:49
				} else {
					//line template/image/index.qtpl:49
					qw422016.N().S(` <span class="muted">--</span> `)
					//line template/image/index.qtpl:51
				}
				//line template/image/index.qtpl:51
				qw422016.N().S(` </td> <td class="align-right"> `)
				//line template/image/index.qtpl:54
				if p.User.ID != i.UserID {
					//line template/image/index.qtpl:54
					qw422016.N().S(` <span class="muted">`)
					//line template/image/index.qtpl:55
					qw422016.N().S(`<!-- Generated by IcoMoon.io -->
<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
<path d="M15.984 12.984c2.344 0 7.031 1.172 7.031 3.516v2.484h-6v-2.484c0-1.5-0.797-2.625-1.969-3.469 0.328-0.047 0.656-0.047 0.938-0.047zM8.016 12.984c2.344 0 6.984 1.172 6.984 3.516v2.484h-14.016v-2.484c0-2.344 4.688-3.516 7.031-3.516zM8.016 11.016c-1.641 0-3-1.359-3-3s1.359-3 3-3 2.953 1.359 2.953 3-1.313 3-2.953 3zM15.984 11.016c-1.641 0-3-1.359-3-3s1.359-3 3-3 3 1.359 3 3-1.359 3-3 3z"></path>
</svg>
`)
					//line template/image/index.qtpl:55
					qw422016.N().S(`</span> `)
					//line template/image/index.qtpl:56
				}
				//line template/image/index.qtpl:56
				qw422016.N().S(` </td> <td class="align-right"> `)
				//line template/image/index.qtpl:59
				if p.User.ID == i.UserID || i.Namespace != nil && i.Namespace.UserID == p.User.ID {
					//line template/image/index.qtpl:59
					qw422016.N().S(` <form method="POST" action="`)
					//line template/image/index.qtpl:60
					qw422016.E().S(i.UIEndpoint())
					//line template/image/index.qtpl:60
					qw422016.N().S(`"> `)
					//line template/image/index.qtpl:61
					qw422016.N().S(string(p.CSRF))
					//line template/image/index.qtpl:61
					qw422016.N().S(` <input type="hidden" name="_method" value="DELETE"/> <button type="submit" class="btn btn-danger">Delete</button> </form> `)
					//line template/image/index.qtpl:65
				}
				//line template/image/index.qtpl:65
				qw422016.N().S(` </td> </tr> `)
				//line template/image/index.qtpl:68
			}
			//line template/image/index.qtpl:68
			qw422016.N().S(` </tbody> </table> `)
			//line template/image/index.qtpl:71
		}
		//line template/image/index.qtpl:71
		qw422016.N().S(` `)
		//line template/image/index.qtpl:72
	}
	//line template/image/index.qtpl:72
	qw422016.N().S(` </div> `)
//line template/image/index.qtpl:74
}

//line template/image/index.qtpl:74
func (p *IndexPage) WriteBody(qq422016 qtio422016.Writer) {
	//line template/image/index.qtpl:74
	qw422016 := qt422016.AcquireWriter(qq422016)
	//line template/image/index.qtpl:74
	p.StreamBody(qw422016)
	//line template/image/index.qtpl:74
	qt422016.ReleaseWriter(qw422016)
//line template/image/index.qtpl:74
}

//line template/image/index.qtpl:74
func (p *IndexPage) Body() string {
	//line template/image/index.qtpl:74
	qb422016 := qt422016.AcquireByteBuffer()
	//line template/image/index.qtpl:74
	p.WriteBody(qb422016)
	//line template/image/index.qtpl:74
	qs422016 := string(qb422016.B)
	//line template/image/index.qtpl:74
	qt422016.ReleaseByteBuffer(qb422016)
	//line template/image/index.qtpl:74
	return qs422016
//line template/image/index.qtpl:74
}

//line template/image/index.qtpl:76
func (p *IndexPage) StreamHeader(qw422016 *qt422016.Writer) {
	//line template/image/index.qtpl:76
	qw422016.N().S(` Images `)
//line template/image/index.qtpl:78
}

//line template/image/index.qtpl:78
func (p *IndexPage) WriteHeader(qq422016 qtio422016.Writer) {
	//line template/image/index.qtpl:78
	qw422016 := qt422016.AcquireWriter(qq422016)
	//line template/image/index.qtpl:78
	p.StreamHeader(qw422016)
	//line template/image/index.qtpl:78
	qt422016.ReleaseWriter(qw422016)
//line template/image/index.qtpl:78
}

//line template/image/index.qtpl:78
func (p *IndexPage) Header() string {
	//line template/image/index.qtpl:78
	qb422016 := qt422016.AcquireByteBuffer()
	//line template/image/index.qtpl:78
	p.WriteHeader(qb422016)
	//line template/image/index.qtpl:78
	qs422016 := string(qb422016.B)
	//line template/image/index.qtpl:78
	qt422016.ReleaseByteBuffer(qb422016)
	//line template/image/index.qtpl:78
	return qs422016
//line template/image/index.qtpl:78
}

//line template/image/index.qtpl:80
func (p *IndexPage) StreamActions(qw422016 *qt422016.Writer) {
	//line template/image/index.qtpl:80
	qw422016.N().S(` <li><a href="/images/create" class="btn btn-primary">Create</a></li> `)
//line template/image/index.qtpl:82
}

//line template/image/index.qtpl:82
func (p *IndexPage) WriteActions(qq422016 qtio422016.Writer) {
	//line template/image/index.qtpl:82
	qw422016 := qt422016.AcquireWriter(qq422016)
	//line template/image/index.qtpl:82
	p.StreamActions(qw422016)
	//line template/image/index.qtpl:82
	qt422016.ReleaseWriter(qw422016)
//line template/image/index.qtpl:82
}

//line template/image/index.qtpl:82
func (p *IndexPage) Actions() string {
	//line template/image/index.qtpl:82
	qb422016 := qt422016.AcquireByteBuffer()
	//line template/image/index.qtpl:82
	p.WriteActions(qb422016)
	//line template/image/index.qtpl:82
	qs422016 := string(qb422016.B)
	//line template/image/index.qtpl:82
	qt422016.ReleaseByteBuffer(qb422016)
	//line template/image/index.qtpl:82
	return qs422016
//line template/image/index.qtpl:82
}

//line template/image/index.qtpl:84
func (p *IndexPage) StreamNavigation(qw422016 *qt422016.Writer) {
//line template/image/index.qtpl:84
}

//line template/image/index.qtpl:84
func (p *IndexPage) WriteNavigation(qq422016 qtio422016.Writer) {
	//line template/image/index.qtpl:84
	qw422016 := qt422016.AcquireWriter(qq422016)
	//line template/image/index.qtpl:84
	p.StreamNavigation(qw422016)
	//line template/image/index.qtpl:84
	qt422016.ReleaseWriter(qw422016)
//line template/image/index.qtpl:84
}

//line template/image/index.qtpl:84
func (p *IndexPage) Navigation() string {
	//line template/image/index.qtpl:84
	qb422016 := qt422016.AcquireByteBuffer()
	//line template/image/index.qtpl:84
	p.WriteNavigation(qb422016)
	//line template/image/index.qtpl:84
	qs422016 := string(qb422016.B)
	//line template/image/index.qtpl:84
	qt422016.ReleaseByteBuffer(qb422016)
	//line template/image/index.qtpl:84
	return qs422016
//line template/image/index.qtpl:84
}
